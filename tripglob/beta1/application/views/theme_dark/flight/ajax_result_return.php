...
<?php if(!empty($flight_result)){ //echo '<pre>';print_r($flight_result);exit('views');
   ...
   for($i=0;$i<$flight_count_result;$i++){
      if ($flight_result[$i]['type']=='onward') {
   ...
   <div class="madgrid">
            ...
            <div class="width80 celhtl">
               ...
               <div class="sector_loop <?=$hide_class?>">
                  <div class="celhtl   width25 midlbord">
                     ...
                     <div class="airlinename"><?php
                        
                     echo $flight_result[$i]['FlightDetails'][$j]['airlineName'][0]; ?>
                        ...
                        <div class="flitruo cloroutbnd">
                           <div class="detlnavi">
                              <div class="col-xs-3  new-cus-se padflt widfty">
                                 ...
                                 echo $flight_result[$i]['FlightDetails'][$j]['DepartureTime'][0];   ?></span></span>
                                 </span>
            ...
            <div class="celhtl new-width20 width20 <?=$hide_class?>">
               <div class="sidepricewrp"  data-price="<?= $this->display_icon.' '. number_format(($flight_result[$i]['amount'] * $this->curr_val), 2); ?>">
                  ...
                  <?php
                     ...
                     ?>
                  <div class="bookbtn">
                     <a class="booknow FlightbookNowreturn" data-target="_blank" data-attr="<?php echo $uid; ?>">Select</a>
                  </div>
               </div>
            </div>
         </div>
      ...
      <div class="mrinfrmtn <?=$hide_class?>">
         <div class="accordion" id="accordion<?=$flight_result[$i]['flight_id'];?>" role="tablist" aria-multiselectable="true">
             ...
             <?php if($flight_result[$i]['paxFareProduct']['IsRefundable'] != 1){
                           ...
                           <div class="detlnavi">
                              ...
                              <div class="col-xs-3 new-div">
                                 ...
                                 //$tot_mins = $flight_result[$i]['FlightDetails']['Segments'][$j][$inner_segment_len]['Duration'];
                                  $tot_mins = round((strtotime($snddate.' '.$sndtime) - strtotime($fdate.' '.$ftime))/3600, 1)*60;
                                  $days = floor($tot_mins / 1440);
                                  $hours = floor(($tot_mins % 1440)/60);
                                  $mins = floor($tot_mins % 60);
                                  if (count($flight_result[$i]['FlightDetails']['Segments'][$j]) != 0 && end($flight_result[$i]['FlightDetails']['Segments'][$j])['AccumulatedDuration'] != 0 ) {
                                     $time_not_one_stop=end($flight_result[$i]['FlightDetails']['Segments'][$j])['AccumulatedDuration'];                                     
                                     $mins=date('i', mktime(0,$time_not_one_stop));
                                     $hours=date('H', mktime(0,$time_not_one_stop));
                                     if ($time_not_one_stop >= 1440) {
                                        $days=1;
                                     }

                                  echo $days.' d: '.$hours.' h: '. $mins.' m' ;
                                  }else{
                                     $time_not_one_stop=current($flight_result[$i]['FlightDetails']['Segments'][$j])['Duration'];
                                     $mins=date('i', mktime(0,$time_not_one_stop));
                                     $hours=date('H', mktime(0,$time_not_one_stop));
                                     if ($time_not_one_stop >= 1440) {
                                        $days=1;
                                 ...
                                 <?php  
                                    if(count($flight_result[$i]['FlightDetails']['Segments'][$j])-1==0){
                                        echo "Non-Stop";
                                    }else{
                                        echo (count($flight_result[$i]['FlightDetails']['Segments'][$j])-1)." "."Stop"; 
                                    }
                                    ?>
                                 </label>
                              </div>
      ...
      <div class="mrinfrmtn <?=$hide_class?>">
         <div class="accordion" id="accordion<?=$flight_result[$i]['flight_id'];?>" role="tablist" aria-multiselectable="true">
            <div class="col-md-12 nopad">
               ...
               <?php 
                 //print_r($flight_result[$i]['nonRefundable']);
               if($flight_result[$i]['paxFareProduct']['IsRefundable'] != 1){
                  $refund_type = 'Refundable';
                  } else{
                  $refund_type = 'Non-Refundable';
                  }
               ...
               <div class="refund pull-right" data-type="<?=$pre_attr?>">
                  <a class="detailsflt fare_flight" data-toggle="collapse">
                  <span><?php echo $refund_type ;//print_r($flight_result[$i]['paxFareProduct']['IsRefundable'] );?></span>
                  </a>
               </div>
            </div>
            <div class="collapse flight_res" id="flight_details_<?php echo  $flight_result[$i]['flight_id'];  ?>" data-parent="#accordion<?php echo  $flight_result[$i]['flight_id'];  ?>">
...
<?php if(!empty($flight_result)){ //echo '<pre>';print_r($flight_result);exit('views');
   $flight_count_result=count($flight_result);
   for($i=0;$i<$flight_count_result;$i++){
       if ($flight_result[$i]['type']=='return') {
...
<script type="text/javascript">
   $(".booknow").click(function(e){
	  ...
	  $(document).ready(function() {
            $('.booknow').click(function() {
                $('.madgrid').toggleClass('main');
                 $('.sector_loop').toggleClass('main');
   ...
   $('#AirlineFilter').html('<?php if(isset($airline_data))
    { 
      $i=1;
      foreach($airline_data as $airline)
         {
            if(in_array($airline->airline_code,$airline_filter))
               {
                  $checked='checked';
               ...
               {
                  $checked='';
               }?><li><div class="squaredThree"><input id="squaredThree<?php echo $i;?>" class="filter_airline" type="checkbox" name="airline" <?=$checked?> value="<?php echo $airline->airline_code;?>" ><label for="squaredThree<?php echo $i;?>"></label></div><label class="lbllbl" for="squaredThree<?php echo $i;?>"><?php echo ucfirst($airline->airline)." (".$airline->airline_code.")";?></label></li><?php $i++; } }?>');
