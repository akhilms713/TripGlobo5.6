...
class Ajax extends CI_Controller {
    ...
    */
    ...
    function bus_list() {
        ...
        // debug($search_params);  debug($search_data); exit;
       // debug($search_data);
        ...
        if ($search_params['op'] == 'load' && intval($search_params['search_id']) > 0 && isset($search_params['booking_source']) == true) {
            ...
            // debug($search_data); exit;
           /* $encode = '{"status":1,"data":{"request":"{\"DateOfJourney\":\"2020-06-05\",\"DestinationId\":\"9573\",\"EndUserIp\":\"157.51.202.168\",\"OriginId\":\"8463\",\"TokenId\":\"1e4b95ef-be62-42f1-aca0-86d5e9d4723e\",\"PreferredCurrency\":\"INR\"}","service_url":"http:\/\/api.tektravels.com\/BookingEngineService_Bus\/Busservice.svc\/rest\/Search\/"}}';
            $json_decode = json_decode($encode); 
            debug($json_decode);
            exit();*/

        // echo"<pre/>";print_r($search_params['booking_source']);exit('hiii');
            switch ($search_params['booking_source']) {
                
                case PROVAB_BUS_BOOKING_SOURCE :
                    $authresponse = $this->bus_lib->auth_tektravel();
                    // echo"<pre/>";print_r($authresponse);
                    if ($authresponse['Status'] == true) {
                        // for update tbo bus table list first truncate the hole table then run it for fresh data.
                        // $city_list_response = $this->bus_lib->get_bus_city_list($authresponse);
                        // $this->bus_model->insert_tbo_city_list($city_list_response);
                      
                        $raw_bus_list = $this->bus_lib->get_bus_list(abs($search_params['search_id']),$authresponse);
                    }
                   ...
                   // debug($search_data_city);
                    ...
                    if ($raw_bus_list['status']) {
                       ...
                       // debug($formatted_search_data);exit('hhhh');
                        ...
                        $currency_obj = new Currency(array('module_type' => 'bus', 'from' => get_application_currency_preference(), 'to' => get_application_currency_preference()));
                        $response['data'] = get_compressed_output(
                                $this->template->isolated_view('bus/travelyaari/travelyaari_search_result', array('currency_obj' => $currency_obj, 'raw_bus_list' => $formatted_search_data, 'search_id' => $search_params['search_id'], 'booking_source' => $search_params['booking_source'], 'search_data_city' => $search_data_city,'tokenId' =>$authresponse['TokenId'], 'TraceId' => $formatted_search_data[0]['TraceId'] ))
                        );
                        // debug($response['data']);exit('hhhh');
                        $response['status'] = SUCCESS_STATUS;
                    }
                    break;

                case PROVAB_TMX_BUS_BOOKING_SOURCE :
                    // debug($search_data); exit;
                   ...
                   // debug($search_data_city);
                    ...
                    if ($raw_bus_list['status']) {
                       ...
                       // debug($formatted_search_data);exit;
                        ...
                        $currency_obj = new Currency(array('module_type' => 'bus', 'from' => get_application_currency_preference(), 'to' => get_application_currency_preference()));
                        $response['data'] = get_compressed_output(
                                $this->template->isolated_view('bus/tmx/travelyaari_search_result', array('currency_obj' => $currency_obj, 'raw_bus_list' => $formatted_search_data, 'search_id' => $search_params['search_id'], 'booking_source' => $search_params['booking_source'], 'search_data_city' => $search_data_city))
                        );
                        $response['status'] = SUCCESS_STATUS;
                    }
                    break;
    ...
    function get_bus_details($filter_boarding_points = false) {
        ...
        if (empty($params['booking_source']) == false and empty($params['search_id']) == false and intval($params['search_id']) > 0) {
            ...
            switch ($params['booking_source']) {
                ...
                case PROVAB_TMX_BUS_BOOKING_SOURCE :
                    $search_data = $this->bus_model->get_search_data($params['search_id']);
                    $search_data_all = json_decode($search_data['search_data'], true);
                    /*debug($search_data);
                    debug($search_data_all); exit();*/
                    $form_data = $this->bus_model->get_bus_station_data_tmx($search_data_all['bus_station_from']);
                    $to_data = $this->bus_model->get_bus_station_data_tmx($search_data_all['bus_station_to']);
                    $bus_info_data = $this->bus_lib->get_route_details($params['search_id'], $params['resultIndex'], $params[2]);
                    $bus_info_data['bus_data']['Form_id'] = $form_data->station_id;
                    $bus_info_data['bus_data']['To_id'] = $to_data->station_id;
                    // debug($form_data);debug($to_data); exit();
                    /*debug($to_data); 
                    debug($bus_info_data); 
                    debug($params); 
                    exit();*/
                    ...
                    // debug($params); exit();
                    $details = $this->bus_lib->get_bus_details($params['resultIndex'], $params['journey_date'], $params['traceId'], $params['ResultToken'], $params['booking_source']);
                    // debug($search_data);
                    // debug($bus_info_data);
                    // debug($details);exit;
                    if ($details['status'] == SUCCESS_STATUS) {
                        ...
                        if ($filter_boarding_points == false) {
                            $response['data'] = get_compressed_output($this->template->isolated_view('bus/tmx/travelyaari_bus_details', $page_data));
                        } else {
                            $response['data'] = get_compressed_output($this->template->isolated_view('bus/tmx/travelyaari_boarding_details', $page_data));
                        }
                        $response['status'] = SUCCESS_STATUS;
                    }
                break;
                ...
                case PROVAB_BUS_BOOKING_SOURCE :
                    ...
                    $boardingDetails = $this->bus_lib->get_bus_boardingDetails($params['EndUserIp'], $params['resultIndex'], $params['traceId'], $params['tokenId']);
                    
                    $currency_obj = new Currency(array('module_type' => 'bus', 'from' => get_api_data_currency(), 'to' => get_application_currency_preference()));
                    $details = $this->bus_lib->get_bus_details($params['EndUserIp'], $params['resultIndex'], $params['traceId'], $params['tokenId'], $params['booking_source'], $currency_obj, 'bus','B2C');
