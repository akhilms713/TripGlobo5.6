<?php
 ...
 if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    /*public function set_credentials() {
        ...
        $this->UserName = 'Goodlife';
        $this->Password = 'Goodlifeapi@123';
        $this->Url = 'http://api.tektravels.com/SharedServices/SharedData.svc/rest/';
    ...
    function set_credentials() {
        ...
        if ($query->num_rows() == 0) {
            ...
            $data['UserName']          = 'Goodlife';
            $data['Password']            = 'Goodlifeapi@123';            
            $data['Url']           = 'http://api.tektravels.com/SharedServices/SharedData.svc/rest/';
            $data['ClientId']           = 'ApiIntegrationNew';
    ...
    public function auth_tektravel()
    {
       $request['data']['service_url'] = $this->Url . 'Authenticate';
       $header_info = $this->get_header();
       $response_data = $GLOBALS['CI']->api_interface->get_aut_json_response($request['data']['service_url'], $header_info);
       // print_r($response_data);exit('firoj');
     ...
     */
    private function bus_search_request($bus_station_from_id, $bus_station_to_id, $bus_date, $number_of_seats = 1, $SearchId = 123456,$authresponse) {
        ...
        // echo "<pre/>";print_r($response);exit('firoj');
        return $response;
    }
     ...
     *
     */
    function GetRouteScheduleDetailsWithComm_request($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source) {
        $response['status'] = SUCCESS_STATUS;
        $response['data'] = array();
    ...
    /*new added */
    function GetBoarding_request($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source) {
        $response['status'] = SUCCESS_STATUS;
        $response['data'] = array();
    ...
    function HoldSeatsForSchedule_request($params) {
        ...
        $url = 'http://api.tektravels.com/BookingEngineService_Bus/Busservice.svc/rest/Block/';
        $Passenger = '';
        $i = 0;
        ...
        foreach ($params['token']['selected_seats'] as $v) {
            ...
            $passenger[$i]['Title'] = $params['pax_title'][$i];
            if ($i == 0 ) {
            ...
            $passenger[$i]['Email'] = $params['email'][$i];
            $passenger[$i]['Age'] = (int) $params['age'][$i];
            $passenger[$i]['FirstName'] = $params['first_name'][$i];
            $passenger[$i]['LastName'] = $params['last_name'][$i];
            // $gender = get_enum_list('gender', $params['gender'][$i]);
            // debug($gender);exit;
            if ($params['gender'][$i] == '1') {
                $passenger[$i]['Gender'] = '1';
            } else if ($params['gender'][$i] == '2') {
            ...
            $passenger[$i]['Phoneno'] = $params['phone_number'][$i];
    ...
    function BookSeats_request($post_params, $blockresponse) {
        ...
        $request_paramas['BoardingPointId'] = $post_params['BoardingPointId'];
        $request_paramas['DropingPointId'] = $post_params['DropingPointId'];
     ...
     * Booking params
     */
    function GetBookSeat_request($booking, $booking_source, $post_params) {
        /*debug($booking);
        debug($post_params); exit();*/
        $response['status'] = SUCCESS_STATUS;
        $response['data'] = array();
    ...
    function get_bus_list($search_id = '', $authresponse='') {
        // debug($search_id);exit('firoj');
        $credentials_data = set_credentials();
        // debug($credentials_data); exit();
        $this->CI->load->driver('cache');
        $response['data'] = array();
        $response['status'] = true;
        $search_data = $this->search_data($search_id);
        // debug($search_data);exit('hiii');
        //Cache
        ...
        if($search_data ==''){
            $search_data = json_decode('{"status":true,"data":{"bus_date_1":"2020-08-11","trip_type":"One Way","bus_station_from":"Bangalore","bus_station_to":"Chennai","from_station_id":"1190","to_station_id":"2553","bus_station_from_id":"8463","bus_station_to_id":"4054"}}', true);
        ...
        if ($search_data['status'] == true) {
            ...
            // if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                $request = $this->bus_search_request($search_data['data']['bus_station_from_id'], $search_data['data']['bus_station_to_id'], $search_data['data']['bus_date_1'], 1, $search_id,$authresponse);
                // debug($request);
                //get data
                if ($request['status']) {
                    $response_data = $GLOBALS['CI']->api_interface->get_json_response($request['data']['service_url'], $request['data']['request'], $header_info);
                    // debug(json_encode($response_data));exit('firoj');
     ...
     */
    //$route_schedule_id, $journey_date, $route_code, $ResultToken, $booking_source
    function get_bus_details($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source, $currency_obj, $module,$module_type='B2B') {
        $response['data'] = array();
        $response['status'] = true;
...
error_reporting(E_ALL);*/
        if (empty($resultIndex) == false and empty($traceId) == false and empty($tokenId) == false) {
            //get request
            $header_info = $this->get_header();
            $request = $this->GetRouteScheduleDetailsWithComm_request($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source);
            // debug($request);
            //get data
            if ($request['status']) {
                ...
                if (valid_array($response_data) == true && $response_data['GetBusSeatLayOutResult']['ResponseStatus'] == true) {
                    // $response['GetBusSeatLayOutResult']['SeatLayoutDetails'] = $response_data['GetBusSeatLayOutResult']['SeatLayoutDetails'];
                    // $SeatDetails = $response_data['GetBusSeatLayOutResult']['SeatLayoutDetails']['SeatLayout']['SeatDetails'][0];
                    ...
                    foreach ($SeatDetails as $seatkey => $SeatDetails_v) {
                        ...
                        foreach($SeatDetails_v as $bus_key => $bus_result){
                            ...
                            /*$response['GetBusSeatLayOutResult']['SeatLayoutDetails'][$bus_key]['TraceId'] = $TraceId;
                            $response['GetBusSeatLayOutResult']['SeatLayoutDetails'][$bus_key]['Origin'] = $Origin;
                            $response['GetBusSeatLayOutResult']['SeatLayoutDetails'][$bus_key]['Destination'] = $Destination;*/
                        }
                    }
    ...
    /*new added */
    function get_bus_boardingDetails($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source) {
        $response['data'] = array();
        $response['status'] = true;
        if (empty($resultIndex) == false and empty($traceId) == false and empty($tokenId) == false) {
            //get request
            $header_info = $this->get_header();
            $request = $this->GetBoarding_request($EndUserIp, $resultIndex, $traceId, $tokenId, $booking_source);
            // debug($request);
            //get data
            if ($request['status']) {

                $response_data = $GLOBALS['CI']->api_interface->get_boarding_json_response($request['data']['service_url'], $request['data']['request'], $header_info);
    ...
    function process_booking($blockresponse, $post_params) {
     /*   debug($blockresponse);
        ...
        if (valid_array($blockresponse)) {
            // echo 'test';exit;
            //get request
            $request = $this->BookSeats_request($post_params, $blockresponse);
            ...
            if ($request['status']) {
                ...
                $GLOBALS['CI']->custom_db->generate_static_response(json_encode($response_data));
                /**    PROVAB LOGGER * */
                $GLOBALS['CI']->private_management_model->provab_xml_logger('Book_Seat', $post_params['book_id'], 'bus', json_encode($request['data']), json_encode($response_data));
                // debug($response_data);exit;
    ...
    function get_booking_details($booking, $booking_source, $post_params, $currency_obj, $module,$module_type='B2B') {
        ...
        if (valid_array($post_params)) {
            ...
            //get request
            $request = $this->GetBookSeat_request($booking, $booking_source, $post_params);
            $header_info = $this->get_header();
            //get data
            if ($request['status']) {
    ...
    public function format_search_response($search_result, $currency_obj, $search_id, $module,$module_type='B2B') {
        ...
        $TraceId  = $search_result['data']['result'][0]['TraceId'];
        $Origin = $search_result['data']['result'][0]['Origin'];
        $Destination = $search_result['data']['result'][0]['Destination'];
        // debug($TraceId); exit();
        $search_result = $search_result['data']['result'][0]['BusResults'];
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            ...
            if ($clean_search_details['status'] == true) {
                ...
                $bus_station_list = $GLOBALS['CI']->db_cache_api->get_bus_station_list(array('k' => 'CityName', 'v' => 'CityId'));
...
error_reporting(E_ALL);*/
    ...
    private function get_header() {
        ...
        $response['EndUserIp'] = $_SERVER['REMOTE_ADDR'];
        return $response;
    }
    ...
    public function get_bus_city_list($authresponse)
    {
 ...
 // debug($res);exit;
    ...
    function debug($ele = array()) {
        ...
        echo '<br>File Name => '.$data[0]['file'].'<br>';
        echo '<br>Line No => '.$data[0]['line'].'<br>';
    }