    ...
    // 		debug($flight);exit;
    		...
    		foreach ($bus as $rowb) {
    			...
    			$aData[$rowb['con_pnr_no']] = array(
    						...
    						'user_name' => $rowb['user_name'],
    						'leadpax' => $rowb['leadpax'],
    						'pnr_no' => $rowb['pnr_no'],
    						'api_tax' => $rowb['api_tax'],
    						'admin_markup' => $rowb['admin_markup'],
    						'agent_markup' => $rowb['agent_markup'],
    						'api_rate' => $rowb['total_cost'],
    						'origin' => $rowb['origin'],
    						'destination' => $rowb['destination'],
    						'mode' => $rowb['mode'],
    		...
    		foreach ($hotel as $rowh) {
    			...
    			$aData[$rowh['con_pnr_no']] = array(
    						...
    						'user_name' => $rowh['user_name'],
    						'leadpax' => $rowh['leadpax'],
    						'pnr_no' => $rowh['pnr_no'],
    						'api_tax' => $rowh['api_tax'],
    						'admin_markup' => $rowh['admin_markup'],
    						'agent_markup' => $rowh['agent_markup'],
    						'api_rate' => $rowh['total_cost'],
    						'origin' => $rowh['origin'],
    						'destination' => $rowh['destination'],
    						'mode' => $rowh['mode'],
	...
	public function view_voucher($Pnr='',$con_pnr_no){
		...
		if($count==1){
            ...
            if($b_data->product_name == 'HOTEL'){
				...
				$data['cart']	= $cart = $this->reports_model->getBookingTemphotel($booking->cart_id);
...
/*end*/
	...
	function update_request_remark(){
	    ...
	    // debug($user_data['user_email']);exit;
	        ...
	        if($request_id){
	            ...
	            $this->general_model->sendMail($_POST['remarks'],$user_data['user_email'],$subject);
	        }
	    }
	...
	function cancel_hotel_booking($pnr,$con_pnr_no)
	{
		...
		if($count==1){
		    ...
		    $data['BookingMode'] = 5;
		    $data['TokenId'] = trim($hotel_data[0]['TokenId'],'"');
		    $data['EndUserIp'] = trim($hotel_data[0]['EndUserIp'],'"');
	...
	function cancel_bus_booking($pnr,$con_pnr_no)
	{
		...
		if($count==1){
		    ...
		    $data['BookingMode'] = 5;
		    $data['TokenId'] = trim($tokenid->token,'"');
		    $data['EndUserIp'] = trim($bus_data[0]['EndUserIp'],'"');
		    $data['BookingId'] = $booking->pnr_no;
		    $data['RequestType'] = '4'; // cancel hotel booking
