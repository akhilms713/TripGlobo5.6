...
class News extends CI_Controller {	
	...
	function show(){
		$data['kb'] = $this->news_model->get_row_list();
	...
	function update($id=''){
		...
		if($_POST)
		{
			$this->news_model->update_row($id, $_POST);
			$data['msg'] = "Successfuly Updated.";
		}
		$data['kb'] = $this->news_model->get_row_single($id);
		$this->load->view('news/update',$data);
	}

	function delete($id=''){
		$this->news_model->delete_row($id);
		$data['msg'] = "Successfuly Updated.";
		redirect(WEB_URL.'news/show','refresh');
	}
	...
	public function change_status() 
	 {
        ...
        if($this->news_model->do_change_status($id, $status))
		{
