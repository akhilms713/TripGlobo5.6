...
class Dashboard extends CI_Controller {
	...
	public function bookings($module = '')
	{
        ...
        $this->new_ajax->initialize($config);
 		...
 		// debug($data['getoverallBookings']);exit;
...
// print_r($data);exit;
	...
	public function mail($pnr_no='',$con_pnr_no='',$booking_global_id=''){	
		...
		if($booking_global_id){
		 ...
		 $count = $this->booking_model->getBookingPnr($booking_global_id,$con_pnr_no)->num_rows();	 
		}else{
        ...
        if($count == 1){
        	...
        	if($booking_global_id){
        		 $b_data = $this->booking_model->getBookingPnr($booking_global_id,$con_pnr_no)->row();	
        	}else{
		...
		// debug($b_data);exit;
        	...
        	if($booking_global_id){
        		$data['b_data'] = $this->booking_model->getBookingPnr($booking_global_id,$con_pnr_no)->row();   
        	}else{
...
// debug($data);exit;
                ...
                $email_type = 'Flight Booking Confirmation letter';
                 // echo "b_data:<pre/>";print_r($data);exit();
                
                $Response = $this->email_model->sendmail_trip($data['email_access'] ,$email_type ,$data['message']);
...
$Response = $this->email_model->sendmail_trip($data['email_access'] ,$email_type ,$data['message']);
                ...
                $response = array('status' => 1);
                //echo json_encode($response);
            }elseif ($b_data->product_id ==8) {
            	if($booking_global_id){
            		 $data['Booking']= $data['b_data']= $this->booking_model->getBookingPnr($booking_global_id,$con_pnr_no)->row();
            		}else{
		         ...
		         $data['booking_agent'] = $passenger = $this->booking_model->getagentbyid($billing_address_id)->result();
		          ...
		          $data['message'] =  $this->load->view(PROJECT_THEME.'/booking/bus_mail_voucher', $data,TRUE);
// debug($data['message']);exit;
		         ...
		         $data['to'] = $data['booking_agent'][0]->billing_email;               
	                ...
	                $data['pnr_no'] = $pnr_no;
	                $data['email_access'] = $this->email_model->get_email_acess()->row();
	                $email_type = 'BUS_BOOKING_VOUCHER';
	                $data['mail_from'] = 'BUS';
	                 $Response = $this->email_model->sendmail_trip($data['email_access'] ,$email_type ,$data['message']);
