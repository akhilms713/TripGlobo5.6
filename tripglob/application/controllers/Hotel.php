...
public function search(){
 ...
 public function generate_rand_no($length = 24)
    ...
    public function call_api($api,$id=''){ 
        ...
        // $session_data = 'RF5RU5ZCMEN8Z25GOLUKPVJV0304101821';
    ...
    // debug($data);exit;    
     ...
     public function hotel_details($hotel_code,$session_id){
       ...
       function get_room_details(){
        ...
        // debug($hotel_info);exit;
        foreach($hotel_info['roomDetails'] as $key_ro =>$room_valu){
        /* agent mark up start*/
        $TotalPrice=$room_valu['Price']['PublishedPriceRoundedOff'];
            ...
            $hotel_info['roomDetails'][$key_ro]['Price']['admin_markup']=$admin_markup;
        }
        // debug($hotel_info);exit;
        /* agent mark up end*/
        ...
        $roomDetails['RoomCombinations'] = $hotel_info['RoomCombinations'];
      ...
      function add_cart(){
            ...
            if (count($_POST) > 0) {
                ...
                $room_details = json_decode(base64_decode($room_details));
                $admin_markup = json_decode(base64_decode($admin_markup));
                $agent_markup_price = json_decode(base64_decode($agent_markup_price));
                $CancellationPolicies = json_decode(base64_decode($CancellationPolicies));
                ...
                $cart_data['RoomTypeCode'] = json_encode($RoomTypeCode);
                $cart_data['RoomTypeName'] = json_encode($RoomTypeName);
                $cart_data['RatePlanCode'] = json_encode($RatePlanCode);
                ...
                $cart_data['room_details'] = json_encode($room_details);
                $cart_data['admin_markup'] = $admin_markup;
                $cart_data['agent_markup'] = $agent_markup_price;
                $cart_data['CancellationPolicies'] = json_encode($CancellationPolicies);
