...
class Booking extends CI_Controller {
	...
	if ($search_id==='domestic_flight') {
            ...
            $count = $this->cart_model->getBookingTemp(current($idd['sessionid']))->num_rows();
			 $data['session_id_c'] = $session_id;	 $data['admin_markup'] = $this->booking_model->get_admin_markup($session_id);           
		    if($count > 0){
                $data['countries'] = $this->general_model->getCountryList();
                $data['book_temp_data'] = $book_temp_data = $this->cart_model->getBookingTemp(current($idd['sessionid']))->result();      
                foreach ($book_temp_data as $key => $value) {
                    $cart_global_id[] = $value->product_name.','.$value->cart_id;
                }
            // debug($cart_global_id);exit;
              	$data['busMarkupval']=$this->bus_model->get_markup_B2C('GENERAL',2);
				...
				else
				{
				  ...
				  $pcode_f = '';
			// debug($secdata); exit;
				  $bt = $this->booking_travellers_conn($secdata,$pcode,$pcode_d,$pcode_f,$search_id="",$search_module="");
				  $data['bt'] = $bt;
        ...
        if(!empty($session_id)){
		    ...
		    if($count > 0){
				...
				else
				{
				  ...
				  $data['bt'] = $bt;
			// debug($data['book_temp_data']); exit;
                $this->load->view(PROJECT_THEME.'/common/booking', $data);
            }else{
            // 	$this->load->view(PROJECT_THEME.'/booking/bus_voucher_view');
                $this->load->view('errors/expiry');
	...
	public function booking_travellers($secdata,$pcode,$pcode_d,$pcode_f,$search_id="",$search_module="") {
        ...
        }else{          //$this->load->view(PROJECT_THEME.'/errors/expiry');

        }
   }

public function booking_travellers_conn($secdata,$pcode,$pcode_d,$pcode_f,$search_id="",$search_module="") {
	     $secdata=json_decode(base64_decode($secdata),1);
		if(isset($secdata) && $secdata!=''){
		 $data['session_id_xc'] = $session_id_xc = $secdata;
		 $session_id_xcc =  json_decode(base64_decode($session_id_xc['s_session_id']),1);
	// debug($session_id_xcc);exit;
				if($session_id_xcc->user_type_id == ''){ $session_id_xcc->user_type_id = 0;}
				if($session_id_xcc!='' && $pcode!='' ) {
					$data['pcode'] = $pcode;
					$data['pcode_d'] = $pcode_d;
					$session_id = current($session_id_xcc['sessionid']);
                    $count = $this->cart_model->getBookingTemp($session_id)->num_rows();
					if($count > 0){
						$data['countries'] = $this->general_model->getCountryList();
						$data['book_temp_data'] = $book_temp_data = $this->cart_model->getBookingTemp($session_id)->result();
    ...
    public function checkout(){
    	...
    	// debug($session_id_xc);exit('under construction');
		$session_id_xcc =  json_decode(base64_decode($session_id_xc));
		...
		if($session_id_xcc!='' && isset($account['user_id']) && $account['user_id']!='' && isset($account['user_type_id']) && $account['user_type_id']!='' ){
			if (!empty($this->session->userdata('user_type'))&& !empty($this->session->userdata('user_id'))) {				
			...
			// debug($status);exit;
				if($status == false){
					$data['status'] = -2;
					$msg=base64_encode('insufficient balance');
        ...
        foreach ($cids as $key => $cid) {
         ...
         if($module == 'FLIGHT'){
               	...
               	if($isCorrent == 'true'){
					...
					if($promo_type == 'PERCENTAGE'){
	    				...
	    				$discount = number_format(($discount) ,2,'.','');
							...
							$BASE_AMOUNT_ = $f_amt;
							$f_amt_v1 = $this->general_model->convert_currecy_with_markup($f_amt,$cart_flight_data->api_id);						$TOTAL = $cart_flight_data->total_cost;
			...
			// debug($DISCOUNT);exit;
				...
				$booking_transaction = array(
					...
					'admin_baseprice' =>$cart_flight_data->admin_baseprice,
 					'agent_markup' => $cart_flight_data->my_markup,
			...
			//	echo '<pre/>';print_r($booking_passenger_v);exit('573');
                ...
                $con_pnr_no = $vid_v.date('m').'F'.date('dHi').$bid;               // debug($con_pnr_no);die;
		  ...
		  if($module == 'HOTEL'){
		 	...
		 	$hotel_request = json_decode(base64_decode(unserialize($hotel_details['request'])),1)  ;
		  	// debug($hotel_request);
		  	// debug($cart_hotel_data);exit;
		 	...
		 	$user_type = $cart_hotel_data->user_type;
				...
				$booking_hotel = array(
												...
												'room_type' 						=> $cart_hotel_data->RoomTypeName,
												'BookingCode'						=> $cart_hotel_data->RatePlanCode,
												'inclusion' 						=> '',
												'cost' 								=> $TOTAL,
                        						'net_cost' 							=> $TOTAL,
                        						'total_cost' 						=> $TOTAL,
												...
												'admin_markup' 						=> $cart_hotel_data->admin_markup,
												'agent_markup' 						=> $cart_hotel_data->agent_markup,
												'org_amt' 							=> $TOTAL,
												'xml_currency' 						=> BASE_CURRENCY,
												'currency_val' 						=> BASE_CURRENCY,
												'city' 								=> $hotel_details['city'],
												'cancel_policy' 					=> $cart_hotel_data->CancellationPolicies,
												'cancel_till_date' 					=> '',
												'cancel_amount' 					=> '',
												'purchase_token' 					=> '',
												'service_val' 						=> '',
												...
												'exp_year' => $checkout_form['exp_year'],
												'cvv_num' => $checkout_form['cvv_num'],
												'cart_hotel_data' => base64_encode(json_encode($cart_hotel_data)),
												'hotel_details' => base64_encode(json_encode($hotel_details)),
                ...
                $bid = $this->booking_model->insert_booking_global_data($booking);
                // debug($p_fname);	
		  		// debug($checkout_form);exit;	
		  	    for ($ad=0; $ad < ($cart_hotel_data->NoOfRooms); $ad++) { 
				$booking_passenger_v=array();	
				
				$p_fname = $checkout_form['first_name'.$cid][$ad];
				$p_lname = $checkout_form['last_name'.$cid][$ad];
				$a_gender = $checkout_form['a_gender'.$cid][$ad];
		  	 ...
		  	 for ($ch=0; $ch < ($cart_hotel_data->NoOfRooms); $ch++) {     	
				if(isset($checkout_form['cfirst_name'.$cid])){
    				$c_fname = $checkout_form['cfirst_name'.$cid][$ch];
    				$c_lname = $checkout_form['clast_name'.$cid][$ch];
    				$c_gender = $checkout_form['c_gender'.$cid][$ch];
				}
            if(isset($c_fname) && is_array($c_fname))
            {
                for($p=0;$p<count($c_fname);$p++)
			...
			if($module == "BUS"){
				...
				if(isset($cart_bus_data) && $cart_bus_data!='')
				{
					...
					$booking_bus = array(
							...
							'selected_seat_response'    => base64_decode($cart_bus_data->selected_seat_response),
					...
					$booking_transaction = array(
						...
						'admin_markup' => $cart_bus_data->admin_markup,	
						'admin_baseprice' =>$cart_bus_data->amount,
	 					'agent_markup' => $cart_bus_data->agent_markup,
	 					'base_amount' => $BASE_AMOUNT_,
						'promo_id' => $promo_id,
						'discount' =>$DISCOUNT,
						'total_amount' => $cart_bus_data->total_cost,
						'card_type' => $checkout_form['card_type'],
						'card_holder_name' => $checkout_form['card_holder_name'],
						'card_number' => $checkout_form['card_number'],
						'exp_month' => $checkout_form['exp_month'],
				...
				// debug($module_count1);exit;
	                $GateURL = WEB_URL.'booking/book/'.$parent_pnr;
	                redirect($GateURL);
	            }elseif($_POST['user_type_name']=='B2C') {
	            	 $GateURL = WEB_URL.'booking/book/'.$parent_pnr;
	                redirect($GateURL);
	            	// debug($ttl);exit;
	            	// $update_booking_payment_getway = array(
	                //     'module' => $module,
					// 	'total' => array_sum($ttl),
	                ...
	                // $this->booking_model->insert_booking_payment_data_payment_getway($update_booking_payment_getway);	            	
	            }
			}
        }
         if($module_count1!="" && $module_count1>1 ){	
            $GateURL = WEB_URL.'booking/flight_availability/'.$parent_pnr;
              $data['status'] = 555;
            $data['GateURL'] = $GateURL;
            redirect($GateURL);
            echo json_encode($data);
	...
	public function promo(){
    	...
    	// debug($promo_data) ; exit();
    		...
    		$exp_time = date('Y-m-d',strtotime($promo_data->expiry_date));
        ...
        //	 print_r($cids);
    			foreach ($cids as $key => $cid) {
		            list($module, $cid) = explode(',', $cid);
		            	...
		            	if($total > $minimum_amount){
		            		if($airline_code!=''&&in_array($airline_code,$multiple_airline)){// echo "promo";exit;
		            			$response['status'] = 1;
		            				if($promo_type == 'PERCENTAGE'){
					    				...
					    				if($total > $promo_data->discount){
					    					$discount = number_format(($discount) ,2,'.','');
											$f_amt =  $total-($discount/count($cids));
					    				$total_payable[] = $f_amt;
						    				$total_discount[] = $discount/count($cids);
					    				}else{
		             ...
		             else if(strtolower($module) ==$module_type&&$module =='BUS')
		             {
		            	$cart = $this->cart_model->getBookingTemp_bus($cid);
						$busMarkupval=$this->bus_model->get_markup_B2C('GENERAL',2);
                      if(($busMarkupval[0]->markup_fixed)>0 && ($busMarkupval[0]->markup_fixed)!='')
                        {
                           ...
                           $busmarkupval= $cart->total_cost*$busmark;
                        }
		            	 $total = $cart->total_cost+$busmarkupval;
		            	if($total > $minimum_amount){
		             ...
		             else if(strtolower($module) ==$module_type&&$module =='HOTEL')
		             {
		        ...
		        if($response['status']==1){
		        	if($promo_type == 'PERCENTAGE'){
	    				...
	    				$response['discMsg'] = 'Promocode is not valid for this flight';
	    				}else{
        ...
        //	 print_r($cids);
    			...
    			foreach ($cids as $key => $cid) {
		            ...
		            list($module, $cid) = explode(',', $cid);
    		            ...
    		            if(strtolower($module) ==$module_type&&$module =='BUS'){
						...
						//	echo "<pre>"; print_r($busMarkupval); echo "</pre>"; die();
	...
	public function flight_availability($parent_pnr,$payment_gateway=''){
		...
		// echo "<pre>"; print_r($global_ids); exit('bp');
			...
			if ($payment_gateway!='success') {				
             if ($global_ids[0]->payment_type=='PAYMENT') {
             	$billing_address_id=$this->db->get_where('payment_gateway_details',array('parent_pnr' => $parent_pnr))->row();
             	
             	$amount=0;
					$billing_address_id=$this->db->get_where('booking_billing_address',array('billing_address_id' => $global_ids[0]->billing_address_id))->row();
					// debug($billing_address_id);exit;
					
					for ($i=0; $i < count($global_ids) ; $i++) { 
						$amount+=$global_ids[$i]->total_amount;
						$discount+=$global_ids[$i]->discount;
					}
					// $this->db->get_where('')
					$booking_flight = array(
					'productinfo' =>'flight',
					'firstname' =>$billing_address_id->billing_first_name,
					'email' => $billing_address_id->billing_email,
					'txnid' =>'TX999'.date('ymdhis'),
					'surl' => '',
					'furl' => '',
					'status' =>'pending',
					...
					'parent_pnr' => $parent_pnr,
					'phone' => explode('-', $billing_address_id->billing_contact_number)[1]
             	...
             	if ($global_ids[0]->discount >= 1 && $global_ids[0]->promo_id >= 1) {
             		...
             		if ($promo_data['limit']=='single') {
             			$data=array(
                        'status'=>'INACTIVE',                        
             			);
             			$this->db->where('promo_id',$global_ids[0]->promo_id);
             			$this->db->update('promo',$data);
             		}elseif ($promo_data['limit']=='multiple') {
             			if ($promo_data['limit_count']==1) {
             				$data=array(
                        'status'=>'INACTIVE',                        
             			);
             			$this->db->where('promo_id',$global_ids[0]->promo_id);
             			$this->db->update('promo',$data);
             				...
             				$data=array(      
             			'status'=>'ACTIVE', 	                                          
                        'limit_count'=>$promo_data['limit_count']-1                       
             			);
             			$this->db->where('promo_id',$global_ids[0]->promo_id);
             			$this->db->update('promo',$data);
             			}
             ...
             if ($global_ids[0]->discount >= 1 && $global_ids[0]->promo_id >= 1) {
         	// debug($global_ids[0]->promo_id);exit;
             		$promo_data=$this->db->get_where('promo',array('promo_id'=>$global_ids[0]->promo_id))->row_array();
             		if ($promo_data['limit']=='single') {
             			$data=array(
                        'status'=>'INACTIVE',                        
...
*/
     		...
     		/*	$SecuritySession1=$pnr_status['soapenv:Envelope']['soapenv:Header']['awsse:Session']['awsse:SessionId'];
                       ...
                       //  echo $bid;
                        $this->Flight_model->Update_Booking_Global($bid, $update_booking, 'FLIGHT');
                           $LocatorCode=$update_booking['pnr_no'];
							...
							//  //   $isdomestic_1=json_decode($global_id->segment_data);
							//  //     $Isdomestic=$isdomestic_1->IsDomestic;
							//       $islcc_1=json_decode($global_id->segment_data);
							//       $islcc=$islcc_1->islcc;
							//   // debug($islcc);exit;
							//       if($islcc==0){
							// 	     $book  = book($global_id,$passenger_info,$billingaddress,$targetfilename);
                            //          $book  = ticket($global_id,$passenger_info,$billingaddress,$targetfilename);
                            //         // debug($book);exit("6909");
							...
							// }else{
							  ...
							  // debug();exit;
							      ...
							      if($islcc==0){							        
    							     $book    = book($global_id,$passenger_info,$billingaddress,$targetfilename);	    
    							     $getResStatus=json_decode($book);
    							     $resStatus= $getResStatus->Response;
    							    if($resStatus->ResponseStatus==1)
    							    {
    							        $book   = ticket($global_id,$passenger_info,$billingaddress,$targetfilename);
    							    }                          
							...
							// }
							// $path = PROJECT_PATH."/beta1/logs/Flight/TBO/Book_Res20240212_155545_3645.txt";					
		                    // $book = file_get_contents($path); //Uncommand 
							$book_data1=json_decode($book,1);
		                    // debug($book_data1);exit;
				...
				if($BookingStatus == 'CONFIRMED'){
					if($global_id->user_type_id == '1' || $global_id->user_type_id == '4') {
						$user_id = $global_id->user_id;
						$this->user_do_payment(($global_id->total_amount-$global_id->discount),$global_id->agent_markup,$user_id);
						$this->user_do_payment_account(($global_id->total_amount-$global_id->discount),$global_id->agent_markup,$user_id,$global_id->booking_global_id,$update_booking['pnr_no']);
    ...
    public function book($parent_pnr,$payment_gateway='success'){	
    	// debug('under construction');exit;
		...
		if($count >= 1){
			...
			$global_ids = $this->payment_model->validate_order_id_org($parent_pnr)->result();
			// debug($global_ids);exit;
		if ($global_ids[0]->product_id==2) {
		 		
			$hotel_data_con = $this->booking_model->getBookingHotelTemp($global_ids[0]->referal_id)->result();
		...
		if ($payment_gateway!='success') {				
             if ($global_ids[0]->payment_type=='PAYMENT') {
             	...
             	$discount=0;
					...
					$booking_flight = array(
					'productinfo' =>'hotel',
					'firstname' =>$hotel_data_con[0]->contact_fname,
					'email' => $hotel_data_con[0]->contact_email,
					'txnid' =>'TX999'.date('ymdhis'),
					'surl' => '',
					'furl' => '',
					'status' =>'pending',
 					...
 					'amount' => $amount,
					'parent_pnr' => $parent_pnr,
					'phone' => $hotel_data_con[0]->contact_mobile_number
					
				);
					// debug($booking_flight);exit;
					$this->db->insert('payment_gateway_details',$booking_flight);
					$id= $this->db->insert_id();    
			...
			$hotel_data_con = $this->booking_model->getbillingaddressTemp($global_ids[0]->billing_address_id)->result();
		...
		if ($payment_gateway!='success') {				
             if ($global_ids[0]->payment_type=='PAYMENT') {
             	...
             	$discount=0;
					...
					$booking_flight = array(
					'productinfo' =>'bus',
					'firstname' =>$hotel_data_con[0]->billing_first_name,
					'email' => $hotel_data_con[0]->billing_email,
					'txnid' =>'TX999'.date('ymdhis'),
					'surl' => '',
					'furl' => '',
					'status' =>'pending',
 					...
 					'amount' => $amount,
					'parent_pnr' => $parent_pnr,
					'phone' => $hotel_data_con[0]->billing_contact_number
			...
			// debug($global_ids);exit;
 			foreach ($global_ids as $key => $global_id) {
				...
				// 			echo "<pre/>";print_r($getPaxDetails);exit('hotel booking');
				...
				// debug($getRoomDetails);exit;
				// debug(json_decode($getRoomDetails[0]->response));exit;
                            $bookingResponse = TBO_final_booking($booking_id,$search_id,$getPaxDetails,$getBlockReq,$hotel_data);
                            // echo $global_id->booking_global_id;
                           // debug($bookingResponse);exit;
                            if($bookingResponse->BookResult->Status == 1){
                                $BookingStatus = 'CONFIRMED';
                                $update_booking_status = array(
										'pnr_no' => $bookingResponse->BookResult->BookingId,  
										'booking_id' => $bookingResponse->BookResult->BookingId,                         
										'booking_status' => $BookingStatus,
										'getRoomDetails' => base64_encode(json_encode($getRoomDetails))

									);
                                // debug($BookingStatus);exit;
                              	if($BookingStatus == 'CONFIRMED'){
								if($global_id->user_type_id == '1' || $global_id->user_type_id == '4') {
								$user_id = $global_id->user_id;
						...
						$this->booking_model->Update_Booking_Global($global_id->booking_global_id, $update_booking_status);	
								...
								// debug($update_booking_status);exit;
						...
						redirect(WEB_URL.'error/booking_failure/'.base64_encode($text)); exit;
							...
							$error_message.='|'.$booking_id.'-Booking data not there';
						redirect(WEB_URL.'error/booking_failure/'.base64_encode($error_message)); exit;
                    	}
                       ...
                       // debug($booking);exit;
								...
								if($get_booking_details['status'] == true){
	                                ...
	                                $update_booking_status = array(
                                      	...
                                      	'booking_id' => $booking['data']['result']['BookResult']['BusId'],
										'booking_status' => $BookingStatus,
										'request_scenario' => json_encode($get_booking_details),
										'booking_res' => json_encode($booking)
									);
  ...
  return $id;
	...
	public function voucher($pnr_no='',$product_id=''){		
        // debug($product_id);exit;     
        $pnr_no = base64_decode(base64_decode($pnr_no));  
        $product_id = base64_decode(base64_decode($product_id));      
        $count = $this->booking_model->getBookingPnr($pnr_no,$product_id)->num_rows();  
        if($count == 1) {
         $b_data = $this->booking_model->getBookingPnr($pnr_no,$product_id)->row();	
     // echo $b_data;die;
        // echo "<pre>"; print_r($b_data); echo "</pre>"; die();
         	$admin_details = $this->booking_model->get_admin_details();
         //	 echo "<pre>"; print_r($admin_details); echo "</pre>"; die();
         	$data['admin_details'] = $admin_details;
            if($b_data->product_id == '1'){
		         $data['b_data'] = $this->booking_model->getBookingPnr($pnr_no,$product_id)->row();
            ...
            if($b_data->product_id == '2'){
					$data['Booking'] = $booking = $this->booking_model->getBookingbyPnr($pnr_no,$product_id)->row();
					$data['Passenger'] = $passenger = $this->booking_model->getPassengerbyPnr($booking->booking_global_id)->result();
					$data['cart']	= $cart =$this->hotel_model->get_hotel_other_details_acount($booking->parent_pnr_no);
					$hotel_data = $this->hotel_model->get_hotel_other_details($cart->HotelCode,$cart->session_id);
					
                    $data['hotel_data'] = $hotel_data[0];
                    $data['request_data'] = $this->db->get_where('search_history',array('origin'=>$booking->search_id))->row();
                    // debug( $data);exit();
					$this->load->view(PROJECT_THEME.'/booking/hotel_voucher_view', $data);
            }
             if($b_data->product_id == '8'){
		         $data['b_data'] = $this->booking_model->getBookingPnr($pnr_no,$product_id)->row();     
		         $data['bus_iterna'] = $this->booking_model->getBookingBusTemp($data['b_data']->referal_id)->row();
		         $data['bus_transaction'] = $this->booking_model->getbookingTransaction($data['b_data']->booking_transaction_id)->row();
		         $booking_global_id=$data['b_data']->booking_global_id;
		         $billing_address_id=$data['b_data']->billing_address_id;
		         $data['Passenger'] = $passenger = $this->booking_model->getPassengerbyid($booking_global_id)->result();
		         $data['booking_agent'] = $passenger = $this->booking_model->getagentbyid($billing_address_id)->result();

				   $data['booking_all_details'] = $this->cart_model->getBookingDetailsBus($data['bus_iterna']->global_id, 'Get_Booking_Det');
				  
				   $data['block_all_details'] = $this->cart_model->getBookingDetailsBus($data['bus_iterna']->global_id, 'Block_Seat');
		          ...
		          // debug($booking_all_details);exit;
	             $this->load->view(PROJECT_THEME.'/booking/bus_voucher_view', $data);
            }
        }else{
        	//$this->load->view(PROJECT_THEME.'/booking/bus_voucher_view');
             $this->load->view('errors/404');
	...
	public function invoice($pnr_no='',$product_id=''){
	
		      $pnr_no = base64_decode(base64_decode($pnr_no));
		      $product_id = base64_decode(base64_decode($product_id));
	              $count = $this->booking_model->getBookingPnr($pnr_no,$product_id);              
	        if($count == 1) {
	           $b_data = $this->booking_model->getBookingPnr($pnr_no,$product_id)->row();
	          // echo '<pre>';print_r($b_data);exit();
	            $admin_details = $this->booking_model->get_admin_details();
         		$data['admin_details'] = $admin_details;
	            if($b_data->product_id == '1'){
					  if ($this->session->userdata('user_id')) {
						...
						if($b_data->user_type_id == $c_user_type)
						{
						   
						  $data['Booking'] = $booking = $this->booking_model->getBookingbyPnr($pnr_no,$b_data->product_id)->row();
						 //  debug($data);exit;
						  $data['tax_json'] = json_decode($data['Booking']->PricingDetails);
						  //debug($data['tax_json']);exit;
						  //$data['tax'] = $data['tax_json'][0]->PriceInfo;
						  $data['tax'] = $data['tax_json']->Tax;
						  //echo $data['tax']; exit;
	             ...
	             // $this->load->view('errors/404');
	              $this->load->view(PROJECT_THEME.'/booking/flight_invoice_view',$datas);
	        }
    }



    public function pre_cancellationTest($pnr){ 
