...
parent::__construct();
		...
		$where.= "";
		$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) as low_cost FROM api_hotel_details t WHERE t.session_id = '$ses_id' $where GROUP BY t.hotel_code $order limit 10 OFFSET $start_pos";
        ...
        if ( $query->num_rows > 0 ) {
			...
			if (empty($fac)) {
				$select2 = "select MIN(low_cost) as minVal, MAX(low_cost) as maxVal from (
					SELECT MIN(t.total_cost) as low_cost FROM api_hotel_details t WHERE t.session_id = '$ses_id' $where group by  t.hotel_code ) as tab limit 10 OFFSET $start_pos";
			} else {
				$select2 = "select MIN(low_cost) as minVal, MAX(low_cost) as maxVal from (
					SELECT MIN(t.total_cost) as low_cost FROM api_hotel_details t WHERE t.session_id = '$ses_id' $where group by  t.hotel_code ) as tab limit 10 OFFSET $start_pos";
			}
    ...
    function getAdminGenericHotelMarkup(){
    	...
    	$this->db->where('markup.markup_type', 'GENERIC'); 
    	$query = $this->db->get('');
    	if ($query->num_rows() == '') {
    	return '';
    	} else {
