...
Class Custom_db extends CI_Model
{
	...
	function get_phone_code_list(){
		$query = 'SELECT origin,country_code,name,iso_country_code FROM api_country_list WHERE country_code!=0 GROUP BY (iso_country_code) ORDER BY iso_country_code';
		return $this->db->query($query)->result_array();
	}
	
	public function get_result_by_query_array($query)
	{
		$result = $this->db->query($query);	
		if($result->num_rows()>0)
		{
