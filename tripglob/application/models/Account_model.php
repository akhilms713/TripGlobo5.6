...
class Account_model extends CI_Model {
  ...
  public function update_credit_amount($update_credit_amount,$b2b_id){
    ...
    function get_hoteldatainfo_new($parent_pnr_no){
    	return $this->db->get_where('booking_hotel',array('parent_pnr'=>$parent_pnr_no))->row_array();
    }
        function get_bus_data($parent_pnr_no){
    	return $this->db->get_where('booking_bus',array('pnr'=>$parent_pnr_no))->row_array();
    }
    function cart_data_bus($id){
    	return $this->db->get_where('cart_bus',array('origin'=>$id))->row_array();
	...
	public function getoverallBookings($user_id, $user_type,$params = array()){
		...
		$this->db->where('booking_global.user_id', $user_id);
			...
			$this->db->group_by('booking_global.parent_pnr_no');
...
$query = $this->db->get();   
   ...
   public function booking_transaction_data($id){
   	return $this->db->get_where('booking_transaction',array('booking_transaction_id'=>$id))->row_array();
   }
