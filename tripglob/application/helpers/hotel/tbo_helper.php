...
//exit("ram");
        ...
        if($getAuthDetails['Error']['ErrorCode']!='0' && empty($getAuthDetails['TokenId'])){
        ...
        // debug($ins_id);exit;
        // $request_data['searchUrl'] = 'http://api.tektravels.com/BookingEngineService_Hotel/hotelservice.svc/rest/GetHotelResult/';
        //live
        $url=set_credentials_hotel(3);
        
        // echo "<pre>";
        // print_r($credentials_data);
        // print_r($url);
        // exit;
        
        $request_data['searchUrl'] = $url['Soap'].'/GetHotelResult';
    ...
    //  debug($search_request);exit("RAHUL");
       ...
       // echo '<pre>';print_r($response['response']);exit();
        ...
        $CI->db->reconnect();
        // $data[0]['session_id']        = '546456456'; //$session_id;
        //         $res = $CI->db->insert('api_hotel_details', $data[0]);
        //         echo $CI->db->last_query();
        //         debug($res); exit('issue checking !!');
    ...
    function tbo_hotel_info($search_id,$hotel_details,$PublishedPrice){
            ...
            //live
            $url=set_credentials_hotel(3);
            $requestUrl             = $url['Soap'].'/GetHotelInfo';
            ...
            //live
            $requestUrl_a                = $url['Soap'].'/GetHotelRoom';
            ...
            // debug($roomResponse_a);exit;
            // GetHotelRoomResult_formation($roomResponse_a);
            $response_data['roomDetails'] = GetHotelRoomResult_formation($roomResponse_a);
            // $response_data['RoomCombinations'] = $roomResponse_a['GetHotelRoomResult']['RoomCombinations'];
    ...
    function tbo_room_list($search_id,$roomReq,$HotelName,$api_id){
       ...
       //live
        $url=set_credentials_hotel(3);
        $requestUrl                 = $url['Soap'].'/GetHotelRoom';
    ...
    function tbo_room_blocking($search_id,$NoOfRooms,$RoomIndex,$RoomTypeCode,$RoomTypeName,$RatePlanCode,$SmokingPreference,$HotelName,$roomRequest,$api_id)
    {
      ...
      //live
        $url=set_credentials_hotel(3);
        $BlockRoomUrl = $url['Soap'].'/BlockRoom';
    ...
    function TBO_final_booking($booking_id,$search_id,$getPaxDetails,$getBlockReq,$hotel_data)
    {
...
// debug(array_column($getBlockReq->HotelRoomsDetails,null, 'RoomTypeCode'));exit;
      ...
      //  $search_data    = $search_history->hotel_search;
        ...
        // echo '<pre>';print_r($search_history->child_age);exit();
       ...
       // foreach ($getBlockReq->HotelRoomsDetails as $key_code => $RoomTypeCode) {
       //      // code...
       // // if ($RoomTypeCode->RoomTypeCode !=$hotel_data[0]->room_code) {
       // //       continue;
       // //   } 
       //  } 

        // debug($getPaxDetails);exit;
     for ($i=0; $i < ($bookReq['NoOfRooms']); $i++) {
        $adtcount = count($getPaxDetails['first_name'.$cart_id][$i]);
        if ($i==0) {
            $count=0;
        }elseif ($i==1) {
        ...
        $child_age=array_slice($search_history->child_age,$count,2);
        // debug($child_age);
        ...
        for ($j=0; $j < $adtcount; $j++) {
            ...
            $isLeadPax = true;
            // : $isLeadPax= false;
            if ($getPaxDetails['first_name'.$cart_id][$i][$j]!='') {
                // code...
        $passengerList[] = array(
                        'Title'                     => $getPaxDetails['a_gender'.$cart_id][$i][$j],
                        'FirstName'                 => $getPaxDetails['first_name'.$cart_id][$i][$j],
                        'MiddleName'                => '',
                        'LastName'                  => $getPaxDetails['last_name'.$cart_id][$i][$j],
                        'Email'                     => null,
        ...
        // echo $getPaxDetails;exit();
        
        for ($k=0; $k < $chdcount; $k++) {
            if ( $getPaxDetails['c_gender'.$cart_id][$i][$k]!='') {
                
                $isLeadPax = false;
                $passengerList[] = array(
                    'Title'                     => $getPaxDetails['c_gender'.$cart_id][$i][$k],
                    'FirstName'                 => $getPaxDetails['cfirst_name'.$cart_id][$i][$k],
                    'MiddleName'                => '',
                    'LastName'                  => $getPaxDetails['clast_name'.$cart_id][$i][$k],
                    'Email'                     =>  null,
    ...
    // echo "<pre/>";print_r($logs_request);die;
       ...
       //live
        $url=set_credentials_hotel(3);
        $BookRoomUrl = $url['Soap'].'/Book';  
        $bookRoomResponse['response'] = get_json_response_hotel($BookRoomUrl,$bookRequest,$api_id,$xml_title);
        // $bookRoomResponse['response'] = file_get_contents('/home/tripglobo/public_html/logs/hotel/tbo/roomBookingInfo_response0304120917.xml');
         // debug($bookRoomResponse['response']);exit;
...
/*testing credential*/
       ...
       $CI=get_instance();
       $token=$CI->db->get_where('authenticate_token',array('type'=>'HOTEL'))->row_array();
...
function GetHotelRoomResult_formation($roomResponse_a){
    // debug($roomResponse_a);exit;
    if (count($roomResponse_a['GetHotelRoomResult']['RoomCombinations']['RoomCombination'])==1) {
       foreach ($roomResponse_a['GetHotelRoomResult']['RoomCombinations']['RoomCombination'][0]['RoomIndex'] as $key_inx => $value_inx) {
        foreach ($roomResponse_a['GetHotelRoomResult']['HotelRoomsDetails'] as $key_val => $value_val) {     
        if ($value_inx==$value_val['RoomIndex']) {
            $value_va[$key_inx][$key_val]=$value_val;            
              }      
        }
        ...
        foreach ($roomResponse_a['GetHotelRoomResult']['RoomCombinations']['RoomCombination'] as $key_inx => $value_inx) {
        foreach ($roomResponse_a['GetHotelRoomResult']['HotelRoomsDetails'] as $key_val => $value_val) {
            if(in_array($value_val['RoomIndex'],$value_inx['RoomIndex'])){
            $value_va[$key_inx][$key_val]=$value_val;
            }
        }
